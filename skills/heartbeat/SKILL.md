...
---
> **${var}** — selector. **Empty (default)** = the ambient fleet check — the live path a cron runs 3×/day; leave it empty for the scheduled run. **`brief`** = the priority brief. See the grammar below.

## Selector / `${var}` grammar

- **`` (empty)** — **Ambient check** across all skills / PRs / issues, and regenerate the public status page. This is the live, scheduled path (08:00 / 14:00 / 20:00 UTC); its behaviour is unchanged. Leave `${var}` empty for the cron.
...
_Source: `articles/token-report-YYYY-MM-DD.md` · verdict: SLIDING_ (illustrative — symbol/figures come from the latest token-report)
